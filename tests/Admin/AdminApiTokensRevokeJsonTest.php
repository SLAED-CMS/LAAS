...
final class AdminApiTokensRevokeJsonTest extends TestCase
{
    public function testRevokeReturnsOk(): void
    {
        $pdo = $this->createBaseSchema();
        $this->seedPermission($pdo, 'api_tokens.revoke', 1);
        $pdo->exec("INSERT INTO api_tokens (id, user_id, name, token_hash, token_prefix, scopes, last_used_at, expires_at, revoked_at, created_at, updated_at)
            VALUES (10, 1, 'CLI', 'hash', 'ABCDEF123456', '[\"admin.read\"]', NULL, NULL, NULL, '2026-01-01 00:00:00', '2026-01-01 00:00:00')");

        $request = $this->makeRequest('POST', '/admin/api-tokens/revoke', [
            'id' => '10',
        ]);
        ...
        $this->assertSame(true, $payload['data']['revoked'] ?? null);
        $this->assertSame('admin.api_tokens.revoke', $payload['meta']['route'] ?? null);
    }
