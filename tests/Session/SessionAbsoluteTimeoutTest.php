...
final class SessionAbsoluteTimeoutTest extends TestCase
{
    public function testAbsoluteTimeoutInvalidatesSession(): void
    {
        $root = sys_get_temp_dir() . '/laas_session_absolute_' . bin2hex(random_bytes(4));
        ...
        $session->set('_session_started_at', time() - 61);

        $factory = new SessionFactory(['driver' => 'native'], null, $root);
        $manager = new SessionManager($root, ['session' => []], $factory);
        $middleware = new SessionMiddleware($manager, ['idle_ttl' => 0, 'absolute_ttl' => 1], null, $session, $root);
