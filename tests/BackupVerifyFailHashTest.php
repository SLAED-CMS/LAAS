...
final class BackupVerifyFailHashTest extends TestCase
{
    public function testVerifyFailsOnHashMismatch(): void
    {
        ...
        $this->assertNotNull($tarPath);
        $altTarPath = $root . '/storage/backups/alt_' . bin2hex(random_bytes(4)) . '.tar';
        rename((string) $tarPath, $altTarPath);

        $tar = new PharData($altTarPath);
        $manifest = json_decode($tar['manifest.json']->getContent(), true);
        $this->assertIsArray($manifest);
        $manifest['files'][0]['sha256'] = str_repeat('0', 64);
        $tar['manifest.json'] = json_encode($manifest, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);

        $tar->compress(Phar::GZ);
        $brokenFile = $altTarPath . '.gz';

        $verify = $manager->verify($brokenFile);
        $this->assertFalse($verify['ok']);
    }

    private function createEnv(): array
    {
        $root = sys_get_temp_dir() . '/laas_backup_verify_fail_' . bin2hex(random_bytes(4));
