...
return new class {
    public function up(\PDO $pdo): void
    {
        ...
        $permId = $this->ensurePermission($pdo, 'users.manage', 'Users manage');
    ...
    public function down(\PDO $pdo): void
    {
        ...
        if ($role) {
            ...
            $stmt->execute(['name' => 'users.manage']);
            $perm = $stmt->fetch();
            if ($perm) {
                $permId = (int) $perm['id'];
        ...
        $pdo->prepare('DELETE FROM permissions WHERE name = :name')->execute(['name' => 'users.manage']);
    }
