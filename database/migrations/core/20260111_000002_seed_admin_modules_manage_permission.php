...
return new class {
    public function up(\PDO $pdo): void
    {
        ...
        $permId = $this->ensurePermission($pdo, 'admin.modules.manage', 'Admin modules manage');
    ...
    public function down(\PDO $pdo): void
    {
        ...
        if ($role) {
            ...
            $stmt->execute(['name' => 'admin.modules.manage']);
            $perm = $stmt->fetch();
            if ($perm) {
                $permId = (int) $perm['id'];
        ...
        $pdo->prepare('DELETE FROM permissions WHERE name = :name')->execute(['name' => 'admin.modules.manage']);
    }
