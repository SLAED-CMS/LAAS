...
return new class {
    public function up(PDO $pdo): void
    {
        ...
        $permId = $this->ensurePermission($pdo, 'rbac.diagnostics', 'RBAC diagnostics');
    ...
    public function down(PDO $pdo): void
    {
        ...
        if ($role) {
            ...
            $stmt->execute(['name' => 'rbac.diagnostics']);
            $perm = $stmt->fetch();
            if ($perm) {
                $permId = (int) $perm['id'];
        ...
        $pdo->prepare('DELETE FROM permissions WHERE name = :name')->execute(['name' => 'rbac.diagnostics']);
    }
