...
return new class {
    public function up(\PDO $pdo): void
    {
        ...
        $permId = $this->ensurePermission($pdo, 'admin.settings.manage', 'Admin settings manage');
    ...
    public function down(\PDO $pdo): void
    {
        ...
        if ($role) {
            ...
            $stmt->execute(['name' => 'admin.settings.manage']);
            $perm = $stmt->fetch();
            if ($perm) {
                $permId = (int) $perm['id'];
        ...
        $pdo->prepare('DELETE FROM permissions WHERE name = :name')->execute(['name' => 'admin.settings.manage']);
    }
